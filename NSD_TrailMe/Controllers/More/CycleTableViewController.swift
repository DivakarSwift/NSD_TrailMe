...
class CycleTableViewController: UITableViewController {
    ...
    let rowTitles = ["Number of Cycles", "Avg. Duration", "Avg. Distance"]
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        loadCycleAllActivities()
        loadCycleWeeklyActivities()
        loadCycleMonthlyActivities()
    }

    func loadCycleAllActivities() {
        cycles.removeAll()
        let query = "Cycle"
        ...
        do {
            cycles =  try CoreDataStack.context.fetch(request)
        ...
        if cycles.count > 0 {
            ...
            cycles.forEach { (cycle) in
                times.append(cycle.duration)
                let formattedDistance = FormatDisplay.distance(cycle.distance)
    ...
    func loadCycleWeeklyActivities() {
        cycles.removeAll()
        let query = "Cycle"
        let today = Date()
    ...
    func loadCycleMonthlyActivities() {
        cycles.removeAll()
        let query = "Cycle"
        let today = Date()
